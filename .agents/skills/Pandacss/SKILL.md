---
name: panda-css-best-practices
description: Panda CSS - recipes, conditions, composition
---
...
> 🚨 **IMPORTANT:** For Panda to successfully track the usage of recipe variants in JSX components, **the prop names used in your React component MUST exactly match the variant names defined in the recipe**. If your recipe has a `visual` variant, your component must receive a `visual` prop. Mapping a `variant` prop to a `visual` recipe variant (e.g. `button({ visual: props.variant })`) will break static tracking, and CSS classes will not be generated!
...
- Due to static nature of Panda, it's not possible to track the usage of the recipes in all cases. Here are some of use
  ...
  **When you change the name of the variant prop in the JSX component**
  ...
  > 🚨 **CRITICAL LIMITATION:** Panda relies on static code analysis to find which variants are used. If you map a React prop to a differently-named recipe variant, Panda cannot determine the possible values at build time, resulting in missing CSS classes.

  In the below example, the `size` prop is renamed to `buttonSize`. Panda will look for a `size` prop on the `<Button>` JSX tag, but will only find `buttonSize`. Therefore, it will not generate the CSS for the size variant.
